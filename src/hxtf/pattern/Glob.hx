...
/**
	...
	`[abc]`  | Matched one of any of the listed characters (the literal character `-` must be escaped like so: `\-`).
	`[a-h]`  | Matches one of any characters in the given range.
	`[!abc]` | Matches one of any characters that are not listed (escape `!` with `\!` if it is at the start of a non-exclusion group).
