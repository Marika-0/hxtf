...
HxTF (**H**a**x**e **T**esting **Framework**) defines a series of 'Test Brokers' (or 'Test Suites') which sequentially instantiate more Test Brokers and/or 'Test Cases'.

An example of a test setup might look like the following:
...
_test/cpp_debug.script_
```sh
./build/cpp/TestRun
```

_test/cpp_optimized.hxml_
```hxml
-cp src
-dce full
-D analyzer-optimize
...
HxTF requires that there be a root class named "TestMain" with a constructor taking no arguments. Beyond that, go nuts.

The general setup has a Test Broker called "TestSuite" in every package, which adds all Test Cases in that package and all Test Brokers one package below. The functions `addTest()` and `addBroker()` are macro's inherited from `hxtf.TestBroker` accepting one absolute dot-path argument to another type.
...
| Define         | Default | Flag | Description |
| -------------- | :-----: | :--: | --- |
| `hxtf_ansi`    | `"1"`   | `-a` | If has the value `"1"`, `hxtf.Print.ansi` is set to `true` and ANSI formatting is not stripped from output printed through `hxtf.Print`, otherwise the opposite occurs. |
| `hxtf_cache`   | `"1"`   | `-z` | If does not have the value `"1"`, the passing tests for the test run will not be cached. |
| `hxtf_cwd`     | $PWD    | N/A  | The working directory that HxTF was invoked from, and the location where cache files are read from and written to. Accessible through `hxtf.TestRun.cwd`. |
| `hxtf_force`   | `"0"`   | `-f` | If has the value of `"1"`, all tests are compiled and run even if they have an entry in the cache file. Accessible through `hxtf.TestRun.forcing` |
| `hxtf_y`       | CLI-defined | `-y` | A colon-separated list of PCRE regexes for tests to push to the test run. |
| `hxtf_n`       | CLI-defined | `-n` | A colon-separated list of PCRE regexes for tests to pull from the test run. |

The value of these flags can be overridden by specific HxTF targets by defining the flag in that targets hxml file. E.g. to never save the cache add the line `-D hxtf_cache=0`, or to always write keep ANSI formatting add the line `-D hxtf_ansi=1`.
...
| softFail(msg:String, printPos = true)        | Prompts the standard error stream with an error-formatted message `msg` and prevents the Test Case from being added to the cache for this target (the Test Case will be rerun until the soft failure doesn't occur), but does not explicitly fail the Test Case. |
...
`hxtf.TestCase` instances also have a _@:noCompletion_ property `_passed(default, never)`. This field is used to more efficiently record if the Test Case has failed instead of setting an internal caching value - modify it during test runs at your own risk.
